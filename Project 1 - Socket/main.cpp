...
std::string request = "GET / HTTP/1.1\r\nHost: thomas.butler.edu:8080\r\n" - could do this, but easier to use objects to do this

*********************

std::string host = "thomas.butler.edu:8080"
std::string path = "/" -- minimal path, will never have no path 
std::string version = "HTTP/1.1"

std::unorderedmap<std::string,std::string> -- for HTTP headers, will be a string, string pair (must included unordered_map header for it to work lol)
// "user=agent"

marshal() -- function to turn all of the above seperate variables into one request (array) as an unsigned char * (unmarshal does the opposite)

marshalling (packing, serializing) -- process of taking all the elements and reforming them into a single unsigned char* to send

socket s
int bytes_sent = s.send(<unsigned char *>, length)
