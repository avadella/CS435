// Write an HTTP Message Object: socket.cpp - Vadella and Baker
...
Socket::Socket(std::string host, int port) : host_(host), port_(port)             // changed/added for web server assignment
{
  socket_ = ::socket(AF_INET, SOCK_STREAM, 0);                                    // create the socket
...
void Socket::connect()
{
  ...
  //if ((rv = getaddrinfo((char *)host_.c_str(), std::to_string(port_).c_str(), &hints, &servinfo)) != 0) {
  if ((rv = getaddrinfo(host_.c_str(), std::to_string(port_).c_str(), &hints, &servinfo)) != 0) {
...
ssize_t Socket::recv(unsigned char *buf, int buf_length)                             // new param added for web server project
{
  //replace rc with buf_length or maybe replace the 512 with buf_length
  ssize_t rc = 0;                                                                    // Actual number of bytes read
  //char buf[512];

  // rc is the number of characters returned

  rc = ::recv(socket_, buf, buf_length, 0);
  buf[rc] = (unsigned char)NULL;                                                     // c-strings are null terminated
...
{
  if (socket_ < 0) {                                                                 // check if socket is valid before doing anything else 
  ...
  memset(&socketInfo, 0, sizeof(socketInfo));                                        // added for web server assignment, clear struct
  
  // all added for web server assignment -- prepare socketInfo struct
  socketInfo.sin_family = AF_INET;                                                   // Address family (IPv4)
  socketInfo.sin_addr.s_addr = INADDR_ANY;                                           // Accept connections from any address
  socketInfo.sin_port = htons(port_);                                                // Set the port (make sure `port_` is set)

  if (::bind(socket_, (struct sockaddr *)&socketInfo, sizeof(socketInfo)) < 0) {     // bind the socket
      ::close(socket_);
      perror("bind");
      exit(EXIT_FAILURE);
  }
